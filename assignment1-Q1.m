...
for it=1:nt % iteration over time
    ...
    for ib=1:nb % iteration across blade number 
        ...
        % saving power and thrust for each timestep as vector, these values
        % are just for one blade
        Power1(it)=Output(it).P;
        Thrust1(it)=Output(it).T;
    end
end

%% save power and thrust

save('Q1_answers.mat', 'Power1','Thrust1','p_n','p_t'); 

%% plot loads

figure;
sizef=16;
plot(r,p_n,'linewidth',1)
ylabel({'Normal Load [Nm]'},'Interpreter','latex','fontsize',sizef);
xlabel({'Radius [m]'},'Interpreter','latex','fontsize',sizef);
print('-depsc','pnQ1');

figure;
plot(r,p_t,'linewidth',1)
ylabel({'Tangential Load [Nm]'},'Interpreter','latex','fontsize',sizef);
xlabel({'Radius [m]'},'Interpreter','latex','fontsize',sizef);
print('-depsc','ptQ1');
